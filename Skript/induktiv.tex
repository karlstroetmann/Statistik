...
\\[0.2cm]
\hspace*{1.3cm}
$\ds\sigma^2 = n \cdot \frac{1}{2} \cdot \Bigl(1 - \frac{1}{2}\Bigr) = 100 \cdot \frac{1}{4} = 25$,
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds p_{C}(x) = \frac{1}{A_1} \cdot \frac{1}{\sqrt{x}} \cdot \exp\left(-\frac{x}{2}\right)$ 
...
\begin{array}[t]{cll}
   & \ds
     ...
     \quad \mbox{mit der Substitution $\ds y = \frac{x}{2}$ wird daraus} 
     \\[0.5cm]
 = & \ds
 ...
 = & \ds
     ...
     \\[0.5cm]
 = & \ds
     - \frac{z}{2} \cdot \exp\left(-\frac{z}{2}\right) - \exp\left(-\frac{z}{2}\right) + 1 \\[0.5cm]
 = & \ds
     1 - \Bigl(\frac{z}{2} + 1\Bigr) \cdot \exp\left(-\frac{z}{2}\right)
\end{array}
$
\\[0.2cm]
...
\begin{array}[t]{lcl}
  P(C \geq 20,\!1) 
& = & \ds
      1 - \left(1 - \Bigl(\frac{z}{2} + 1\Bigr) \cdot \exp\left(-\frac{z}{2}\right)\right)
      \\[0.3cm]
& = & \ds
...
hin, dass die Null-Hypothese falsch ist.  Wir verwerfen daher die Null-Hypothese und
sehen, dass sich die Wahrscheinlichkeit der einzelnen
Zahlen bei meinem Würfel deutet von dem Wert $\frac{1}{5}$ unterscheidet. \eoxs
\vspace*{0.3cm}
...
Raucher, die nicht erkrankt sind mit $X_{2,1}$, die Anzahl
der erkrankten Nichtraucher mit $X_{1,2}$ und die Anzahl der Nichtraucher, die nicht erkrankt sind, mit
...
\\[0.2cm]
\hspace*{0.3cm}
$\ds C = \frac{\bigl(X_{1,1} - N_1 \cdot p\bigr)^2}{N_1 \cdot p} + \frac{\bigl(X_{2,1} - N_1 \cdot (1-p)\bigr)^2}{N_1 \cdot (1 - p)} + 
         \frac{\bigl(X_{1,2} - N_2 \cdot p\bigr)^2}{N_2 \cdot p} + \frac{\bigl(X_{2,2} - N_2 \cdot (1-p)\bigr)^2}{N_2 \cdot (1 - p)}$
\\[0.2cm]
...
\begin{enumerate}
\item Wir haben vier Zufalls-Variablen $X_{1,1}$, $X_{2,1}$, $X_{1,2}$ und $X_{2,2}$.
\item Zwischen diesen vier Zufalls-Variablen gibt es drei verschiedene Beziehungen:
      \begin{enumerate}
      \item $X_{1,1} + X_{2,1} = N_1$
      \item $X_{1,2} + X_{2,2} = N_2$
      \item $\ds\frac{X_{1,1} + X_{1,2}}{N_1 + N_2} = p$
      \end{enumerate}
\end{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds P(C \geq z) = 1 - \int_0^z \frac{1}{\sqrt{2\pi\cdot x\;}} \cdot \exp\left(-\frac{x}{2}\right) \, \dx x
                 = 2 \cdot \Phi\bigl(-\sqrt{z}\bigr)
$
\\[0.2cm]
gefunden.  Mit $z = 79,\!81$ gilt 
\\[0.2cm]
\hspace*{1.3cm}
...
eine \blue{kausale} Beziehung besteht, denn die beobachtete \blue{Korrelation} könnte auch andere
Ursachen haben:
\begin{enumerate}
...
\item Es wäre theoretisch möglich, dass es ein Gen gibt, dass einerseits das Auftreten von
      ...
      Spaßfaktor wie Lungenkrebs. \eoxs
\end{enumerate}
...
\begin{Satz}[$\chi^2$-Test]
  ...
  \\[0.2cm]
  einer $\chi^2$-Verteilung mit $n - k$ Freiheitsgraden:
  \\[0.2cm]
  \hspace*{1.3cm}
  $C \sim \chi^2(n-k)$.
  \\[0.2cm]  
  Nimmt die Zufalls-Variable also
  einen bestimmen Wert $z$ an, so gilt für die Wahrscheinlichkeit, dass $C \geq z$ ist
  \\[0.2cm]
  \hspace*{1.3cm}
  $\ds P(C \geq z) = 1 - \int_0^z p_{C}(x)\,\dx x$. 
  \\[0.2cm]
