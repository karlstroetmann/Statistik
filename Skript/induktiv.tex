...
& = & \ds
      ...
      \\[0.5cm]
& = & \ds
      1 - 2 \cdot \frac{1}{\sqrt{2\cdot\pi\,}} \cdot \int_0^{\sqrt{z}} \cdot \exp\left(-y^2/2\right)\,\dx y
      \\[0.5cm]
& = & \ds
      1 - 2 \cdot \left( \frac{1}{\sqrt{2\cdot\pi\,}} \cdot \int_{-\infty}^{\sqrt{z}} \cdot \exp\left(-y^2/2\right)\,\dx y
                        -\frac{1}{\sqrt{2\cdot\pi\,}} \cdot \int_{-\infty}^{0} \cdot \exp\left(-y^2/2\right)\,\dx y\right)
      \\[0.5cm]
& = & \ds
      1 - 2 \cdot \left( \frac{1}{\sqrt{2\cdot\pi\,}} \cdot \int_{-\infty}^{\sqrt{z}} \cdot \exp\left(-y^2/2\right)\,\dx y
                        -\frac{1}{2}
      ...
      \\[0.5cm]
& = & \ds
      2 - 2 \cdot  \frac{1}{\sqrt{2\cdot\pi\,}} \cdot \int_{-\infty}^{\sqrt{z}} \cdot \exp\left(-y^2/2\right)\,\dx y
      \\[0.5cm]
  & = & 2 - 2 \cdot \Phi\bigl(\sqrt{z}\bigr) \\[0.2cm]
  & = & 2 - 2 \cdot (1 - \Phi\bigl(-\sqrt{z}\bigr) \\[0.2cm]
  & = & 2 \cdot \Phi\bigl(-\sqrt{z}\bigr) = 2 \cdot \Phi(-5) \approx 5.7 \cdot 10^{-7}
\end{array}
$
\\[0.2cm]
Wir können den Wert der Funktion $\Phi(x)$ mit \textsc{SetlX} berechnen, denn es gilt
\\[0.2cm]
\hspace*{1.3cm}
...
\example
Die Würfel, die an der DHBW zur Notenfindung eingesetzt werden, werden jedes
...
einzelnen Ziffern wird notiert.  Da an der DHBW nur Noten von $1$ bis $5$ vergeben werden,
...
\item Es wäre theoretisch möglich, dass es ein Gen gibt, dass einerseits das Auftreten von
      ...
      Zeitpunkt, an dem Sir Ronald diese Hypothese aufstellte, der medizinische
