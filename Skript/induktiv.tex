...
\begin{array}[t]{cl}
 ...
 = & \ds
     ...
     \\[0.5cm]
 = & \ds
     \sum\limits_{i=1}^{n} \bigl(X_i - \mu\bigr)^2 + \bigl(\mu - \overline{X}\bigr)^2 + 2 \cdot (\mu - \overline{X}) \cdot (X_i - \mu)
     \\[0.5cm]
 = & \ds
...
\\[0.2cm]
denn die Zufalls-Variablen $X_i$ haben ja alle die selbe Verteilung wie die Zufalls-Variable $X$,
...
$\frac{1}{\sigma} \cdot (X_i - \mu)$ ist normal verteilt mit Erwartungswert
$0$ und Varianz $1$, also standard-normal-verteilt.
...
Nach Aufgabe 29 ist die Summe zweier normal verteilter Zufalls-Variablen normal verteilt.
...
$\frac{1}{n} \cdot \sigma^2$.  Also hat die Zufalls-Variable $\frac{\sqrt{n}}{\sigma} \cdot (\overline{X} - \mu)$ den Erwartungswert 0 und die Varianz
...
Ohne Beweis teilen wir mit, dass die Zufalls-Variablen U und V unabhängig sind.
...
die Ergebnisse ``Wappen'' und ``Zahl'' mit der selben Wahrscheinlichkeit einstellen.
...
\\[0.2cm]
wie stark $X_0$ und $X_1$ von dem Erwartungswert $n \cdot \frac{1}{2}$ abweichen.  
Eine solche Zufalls-Variable wird auch als eine \blue{Statistik} bezeichnet.
\index{Statistik}
...
Für $n = 100$  haben wir $\sigma^2 = 25$, so dass diese Bedingung erfüllt ist.
Wenn $X_0$ aber ein Normal-Verteilung hat, so hat die Zufalls-Variable
\\[0.2cm]
\hspace*{1.3cm}
$\ds Y := \frac{X_0 - n \cdot \frac{1}{2}}{\sqrt{n \cdot \frac{1}{2} \cdot (1-\frac{1}{2})}}$
\\[0.2cm]
eine Standard-Normal-Verteilung $N_{0,1}$.  Im letzten Kapitel haben wir gesehen, dass
dann $Y^2$ eine $\chi^2$-Verteilung mit einem Freiheitsgrad genügt.  Die Zufalls-Variable
...
mit einem Freiheitsgrad hatten wir im letzten Kapitel den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
$\ds p_{\chi^2,1}(x) = \frac{1}{A_1} \cdot \frac{1}{\sqrt{x}} \cdot \exp\left(-\frac{x}{2}\right)$ 
\quad mit $\ds A_1 = \sqrt{2\;} \cdot \Gamma\left(\frac{1}{2}\right) = \sqrt{2\pi\;}$
\\[0.2cm]
...
& = & \ds
      ...
      \\[0.5cm]
&   & \mbox{Substitution $x = 2\,y^2$, also $\dx x = 4 \cdot y\, \dx y$}
      \\[0.2cm]
& = & \ds
      1 - \int_0^{\sqrt{z/2}} \frac{1}{\sqrt{4\pi\;} \cdot y} \cdot
      \exp\left(-y^2\right)\cdot 4 \cdot y \, \dx y
      \\[0.5cm]
& = & \ds
      1 - \frac{2}{\sqrt{\pi\,}} \cdot \int_0^{\sqrt{z/2}} \cdot \exp\left(-y^2\right)\,\dx y
      \\[0.5cm]
& = & \ds
      1 - \textsl{erf}\left(\sqrt{\frac{z}{2}}\right)
 = 
      1 - \textsl{erf}\left(\frac{5}{\sqrt{2\,}}\right)
\approx 
      5.7 \cdot 10^{-7}
\end{array}
$
...
Zufalls-Variable $C$ einen Wert größergleich $25$ annimmt, 
verschwindend gering.  Daher verwenden wir die Null-Hypothese verwerfen.
\qed
...
\example
Die Würfel, die an der Berufsakademie zur Notenfindung eingesetzt werden, werden jedes
Jahr in einem aufwendigen Prozess\footnote{
Unter den Studenten ist dieser Prozess als die sogenannte \blue{Assistenten-Prüfung} bekannt.} 
geeicht.  Dazu wird mit dem zu eichenden Würfel 100 mal gewürfelt und die Häufigkeit der
einzelnen Ziffern wird notiert.  Da bei der BA nur Noten von $1$ bis $5$ vergeben werden,
handelt es sich bei den verwendeten Würfeln um kostspielige Spezialanfertigungen, mit
denen sich nur Zahlen aus der Menge $\{1,\cdots,5\}$ würfeln lassen.
Bei meinem Würfel ergab sich die dabei die folgenden Verteilung der Häufigkeiten:
\begin{center}
...
\end{tabular}
\end{center}
Wir würden erwarten, dass jede Zahl mit einer durschnittlichen Häufigkeit von $100 \cdot \frac{1}{5} = 20$
...
der Zahl $i$ mit $X_i$. 
Als ein Mass für die Abweichung der oben gezeigten Häufigkeiten
...
Zahlen bei meinem Würfel deutlich von dem Wert $\frac{1}{5}$ unterscheidet.  
Also sollte ich mir von den Studiengebühren einen neuen Würfel beschaffen. \qed
\vspace*{0.3cm}
...
$
\\[0.2cm]
Das Integral, das hier auftritt, haben wir bereits zu Beginn dieses Abschnitts berechnet.
Es gilt also 
\\[0.2cm]
\hspace*{1.3cm}
$\ds P(C \geq z) \approx 1 - \textsl{erf}\left(\sqrt{\frac{z}{2}}\right) \approx 0.41 \cdot 10^{-18}$
\\[0.2cm]
...
\item Es wäre theoretisch möglich, dass es ein Gen gibt, dass einerseits das Auftreten von
      ...
      Einer der größten Statistiker des letzten Jahrhunderts, Sir Ronald Fisher (1890 -
      1962), war tatsächlich der Ansicht, dass die
      ...
      Zeitraum, an dem Sir Ronald diese Hypothese aufstellte, der medizinische
      ...
      Und um die Geschichte abzuschließen, bemerken wir noch, dass Sir Ronald
...
\noindent
Wir fassen die bisherigen Beispiele in einem Satz zusammen, den wir allerdings nicht
beweisen können.
...
\\[0.2cm]
\hspace*{1.3cm}
\texttt{http://www.cemetery.org/alphaindex.html}
\\[0.2cm]
...
dass für den Todestag eines Menschen alle Wochentage die selbe Wahrscheinlichkeit haben
...
die selbe Wahrscheinlichkeit haben.  Damit hat die Wahrscheinlichkeit, dass eine bestimmte
