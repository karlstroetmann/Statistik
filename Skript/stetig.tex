...
\begin{Definition}[\href{https://de.wikipedia.org/wiki/Logistische_Verteilung}{Logistische Verteilung}]
  ...
  \\[0.2cm]
  \hspace*{1.3cm}
  $\ds F_X(x) = \frac{1}{\ds 1 + \exp\Bigl(-\frac{x-\mu}{\sigma}\Bigr)}$
  \\[0.2cm]
  hat.  In diesem Fall schreiben wir
  $X \sim \mathrm{Log}(\mu, \sigma)$.  \eox
\end{Definition}
...
\noindent
\textbf{Hinweis}:
Die Berechnung des Erwartungswerts der logistischen Verteilung verläuft ohne Komplikationen.  Bei der
...
\\[0.2cm]
Diese Formel müssen Sie an geeigneter Stelle rückwärts, also von rechts nach links, anwenden.  Schließlich
sollten Sie sich daran erinnern, dass wir am Ende des zweiten Semesters die Formel
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
herleiten und die Varianz der logistischen Verteilung können Sie auf diese Summe zurück führen.
\eoxs
...
& = & \ds 
      ...
      \\[0.5cm]
& = & \ds 
      \int_a^b \sum\limits_{n=0}^\infty \frac{(t\cdot x)^n}{n!} \cdot p_X(t)\, \dx x & \quad
