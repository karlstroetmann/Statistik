...
\\[0.2cm]
Sind $a,b \in \mathbb{R}$ mit $a < b$ und ist $X:\Omega \rightarrow \{a,b\}$ eine Zufalls-Variable, für die
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
und die Zufalls-Variable $X$, welche die Zahl in Minuten angibt, bis der nächste Bus kommt, ist
...
$n$ groß und damit die Intervall-Länge $x_{k} - x_{k-1} = \frac{d-c}{n}$ klein ist.  Dann haben wir 
\\[0.2cm]
\hspace*{1.3cm}
...
Die \blue{Varianz} einer stetigen Zufalls-Variable wird nun wie früher durch 
\\[0.2cm]
\hspace*{1.3cm}
...
\exercise
Berechnen Sie Erwartungswert und Varianz der Zufalls-Variable $X$, falls für die Verteilungs-Funktion
$F_X$ gilt
\\[0.2cm]
\hspace*{1.3cm}
$F_X(t) = \left\{
...
\begin{Definition}[\href{https://de.wikipedia.org/wiki/Logistische_Verteilung}{Logistische Verteilung}]
  ...
  $X \sim \mathrm{Log}(\mu, s)$.  \eox
\end{Definition}

\remark
Die logistische Verteilung spielt später in der \href{https://de.wikipedia.org/wiki/Logistische_Regression}{logistischen Regression}
und als \blue{Aktivierungsfunktion} bei den \href{https://de.wikipedia.org/wiki/Künstliches_Neuron}{Neuronen} eines
\blue{neuronalen Netzes} eine wichtige Rolle. \eoxs

\begin{figure}[t]
  \centering
   \epsfig{file=Abbildungen/logistic,scale=1.0}
...
\begin{enumerate}[(a)]
\item Es ist am einfachsten, wenn Sie zunächst den Erwartungs-Wert und die Varianz einer Zufalls-Variable $X$
      berechnen, für die $X \sim \mathrm{Log}(0,1)$ gilt.  Ist $Y$ dann eine Zufalls-Variable, für die
      ...
      \\[0.2cm]
      definieren.  Mit Hilfe des Verschiebungs-Satzes können Sie anschließend $E[Y]$ und $\mathrm{Var}[Y]$ bestimmen.
...
\item Die Berechnung des Erwartungswerts $E[X]$ verläuft ohne Komplikationen.  Bei der
      Berechnung der Varianz sollten Sie sich an die Formel für die geometrische Reihe erinnern:
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \\[0.2cm]
      Diese Formel müssen Sie an geeigneter Stelle rückwärts, also von rechts nach links, anwenden.  
\item Schließlich sollten Sie sich daran erinnern, dass wir am Ende des zweiten Semesters die Formel
      \\[0.2cm]
      \hspace*{1.3cm}
      $\ds \sum\limits_{n=1}^\infty \frac{1}{n^2} = \frac{\;\pi^2}{6\,}$
      \\[0.2cm]
      bewiesen haben.  Aus dieser Formel lässt sich die Summenformel
      \\[0.2cm]
      \hspace*{1.3cm}
      $\ds \sum\limits_{n=1}^\infty \frac{(-1)^{n+1}}{n^2} = \frac{\;\pi^2}{12\;}$
      \\[0.2cm]
      herleiten und $\mathrm{Var}[X]$ kann auf diese Summe zurück geführt werden.
      \eoxs
\end{enumerate}
...
\\[0.2cm]
Eine solche Zufalls-Variable heißt \href{https://de.wikipedia.org/wiki/Normalverteilung}{normal-verteilt} mit
Parametern $\mu$ und $\sigma$, wir schreiben 
\\[0.2cm]
\hspace*{1.3cm}
$X \sim \mathcal{N}(\mu, \sigma)$.
\\[0.2cm]
Wir werden später sehen, dass $\mu$ der Erwartungswert von $X$ ist.  Den
...
haben wir für die Wahrschein\-lichkeits-Dichte $p_Y$:
\\[0.2cm]
\hspace*{1.3cm}
...
Es seien  $X:\Omega \rightarrow \mathbb{R}$ und $Y: \Omega \rightarrow \mathbb{R}$ zwei \red{unabhängige} Zufalls-Variablen.
Dann lässt sich die moment-erzeugende Funktion der Zufalls-Variable $X + Y$ als Produkt der
...
des Produkts unabhängiger Zufalls-Variablen $X$ und $Y$ die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$E[X^n] = f^{(n)}(0)$ \quad für alle $n \in \mathbb{N}$ gilt.  \qeds

\remark
An dieser Stelle sehen wir, wie nützlich die Kenntnis der Moment-erzeugenden Funktion einer Zufalls-Variable
...
\begin{Satz}[Momente der Standard-Normal-Verteilung] \hspace*{\fill} \linebreak
  ...
  $X\sim\mathcal{N}(0,1)$.  Dann können die Momente von $X$ wie folgt berechnet werden:
  \begin{enumerate}
   \item $\mathrm{Mt}(X,2\cdot n + 1) = 0$ \quad für alle $n\in\mathbb{N}$.
   \item $\mathrm{Mt}(X,2\cdot n) = (2\cdot n-1)!!$ \quad für alle $n\in\mathbb{N}$ mit $n > 0$.

         Dabei ist für eine ungerade Zahl $2 \cdot m + 1$ der Ausdruck $(2\cdot m+1)!!$ als
         \\[0.2cm]
         \hspace*{1.3cm}
         ...
         $
         \\[0.2cm]
         definiert.  Dieser Ausdruck wird auch als \href{https://en.wikipedia.org/wiki/Double_factorial}{Doppel-Fakultät} bezeichnet.
 \end{enumerate}
\end{Satz}

\proof
Wir haben früher für die Moment-erzeugende Funktion $M_X(t)$ einer standard-normal-verteilten Zufalls-Variable
$X$ den Ausdruck
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
schreiben lässt.  Setzen wir hier für $x$ den Ausdruck $t^2/2$ ein, so erhalten wir
\\[0.2cm]
\hspace*{1.3cm}
...
$,
\\[0.2cm]
denn Sie können durch eine elementare vollständige Induktion zeigen, dass
\\[0.2cm]
\hspace*{1.3cm}
...
Es sei $(X_n)_{n\in\mathbb{N}}$ eine Folge unabhängiger stetiger Zufalls-Variablen, die alle dieselbe Wahrscheinlichkeits-Dichte haben,
es gilt also 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\mu = E[X_n]$ \quad und \quad $\sigma^2 = \var[X_n]$ \quad für alle $n \in \mathbb{N}$.
\\[0.2cm]
Definieren wir für $n \in \mathbb{N}$ die Zufalls-Variablen $S_n$ durch 
\\[0.2cm]
\hspace*{1.3cm}
...
moment-erzeugenden Funktionen $M_{Y_k}$ für alle $k$ gleich:
\\[0.2cm]
\hspace*{1.3cm}
$\forall k,l \in \mathbb{N}: f_k = f_l$.
\\[0.2cm]
Daher können wir den Index $k$ bei der 
Funktion $f_k$ weglassen und 
\\[0.2cm]
\hspace*{1.3cm}
$f := f_k$
\\[0.2cm]
definieren.  Für die moment-erzeugende Funktion $M_{S_n}$ finden wir jetzt das Folgende:
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[b]{lcl}
...
&   & \mbox{denn die Zufalls-Variablen $X_k$ sind unabhängig} \\[0.2cm]
& = & \ds
...
vernachlässigt werden können.  Daher haben wir 
\\[0.2cm]
\hspace*{1.3cm}
$
\begin{array}[b]{lcl}
...
\\[0.2cm]
ist aber gerade die moment-erzeugende Funktion einer Zufalls-Variable $X$, für die $X \sim \mathcal{N}(0,1)$
gilt.  Aus dem Eindeutigkeits-Satz \ref{satz:moment-eindeutig} für die moment-erzeugenden Funktionen folgt nun 
\\[0.2cm]
\hspace*{1.3cm}
...
Zufalls-Variablen beschränkt sind. Darüber hinaus muss dann noch eine weitere recht technische Bedingung erfüllt sein.
Der zentrale Grenzwertsatz ist der Grund dafür, dass in der Praxis viele Zufalls-Variablen normal-verteilt sind.
...
\\[0.2cm]  
Außerdem seien $X_1$ und $X_2$ \red{unabhängig}. 
...
In diesem Abschnitt gehen wir davon aus, dass $X_1$, $X_2$, $\cdots$, $X_n$  \red{unabhängige} 
Zufalls-Variablen sind, die alle einer standardisierten Normalverteilung genügen, für die einzelnen
Zufalls-Variablen $X_k$ gilt also $X_k \sim \mathcal{N}(0, 1)$ für alle $k=1,\cdots,n$.  Diesen Sachverhalt können wir auch als
\\[0.2cm]
\hspace*{1.3cm}
$\ds P(X_k \leq x) = \frac{1}{\sqrt{2\,\pi\,}} \cdot \int_{-\infty}^x
\exp\biggl(-\frac{t^2}{2}\biggr)\mathrm{d} t$ \quad für alle $k=1,\cdots,n$
\\[0.2cm]
...
\href{https://de.wikipedia.org/wiki/Fehlerintegral}{gaußsche Fehlerintegral}
...
$
\\[0.2cm]
Aus $\Phi(x) + \Phi(-x)$ folgt  $\Phi(-x) = 1 - \Phi(x)$ und damit finden wir für die Verteilungs-Funktion von $X^2$ die Gleichung 
\\[0.2cm]
\hspace*{1.3cm}
...
Die \blue{Gamma-Funktion} ist für positive reelle Zahlen $x$ als das folgende Integral definiert:
\\[0.2cm]
\hspace*{1.3cm}
...
\item $\Gamma(n+1) = n!$ \quad für alle $n\in\mathbb{N}$.
      ...
      der Fakultäts-Funktion auf die positiven reellen Zahlen aufgefasst werden kann. 
      \eoxs
\end{enumerate}
...
\begin{Satz} \label{satz:chi-square-normal}
  ...
  $\ds A_n = 2^{n/2} \cdot \Gamma\left(\frac{n}{2}\right)$.
  \\[0.2cm]
Zusammenfassend können wir auch sagen: Ist $Z$ eine Zufalls-Variable, so gilt
\\[0.2cm]
\hspace*{1.3cm}
$\ds Z \sim \chi^2(n) \;\Rightarrow\; Z \sim \Gamma\Bigl(\frac{n}{2},2\Bigr)$. \eox
\end{Satz}

Die Abbildungen \ref{fig:chiSquare1.png}, \ref{fig:chiSquare2.png}, \ref{fig:chiSquare34510.png} und \ref{fig:chiSquare30.png} auf den folgenden
Seiten zeigen die Wahrscheinlichkeits-Dichten der
$\chi^2$-Verteilungen für die Freiheitsgrade $1$, $2$, $3$, $4$, $5$, $10$ und $30$.  Wir sehen, dass sich diese
Wahrscheinlichkeits-Dichten mit wachsender Zahl von Freiheitsgraden an die Wahrscheinlichkeits-Dichte einer
Normalverteilung annähern. Bei der Diskussion des $\chi^2$-Tests im nächsten Kapitel benötigen wir die beiden
folgenden Sätze.
...
$\chi^2$-Verteilung mit $m+n$ Freiheitsgraden genügt.   Daher folgt die Behauptung aus
dem Eindeutigkeits-Satz \ref{satz:moment-eindeutig}. \qeds

\remark
Der obige Satz lässt sich leicht auf die Summe von mehr als zwei
...
\exercise
Nehmen Sie an, dass die Zufalls-Variable $Z$ einer $\chi^2$-Verteilung mit $n$ Freiheitsgraden genügt.
Berechnen Sie den Erwartungswert und die Varianz von $X$.

\noindent
\textbf{Hinweis}: Berechnen Sie zunächst den Erwartungswert und die Varianz einer Zufalls-Variable $Z$, für die
$Z \sim \chi^2(1)$ gilt.
\eoxs

\exercise
Es sei $X$ eine Zufalls-Variable, für die $X \sim \chi^2(n)$ gilt.  An welcher Stelle nimmt die
