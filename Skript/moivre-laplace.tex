\chapter[Berechnung der Binomial-Koeffizienten]{Berechnung von Fakultäten und Binomial-Koeffizienten}
...
\href{https://en.wikipedia.org/wiki/Cornelius_Lanczos}{Cornelius Lanczos} (1893 - 1974). 
Die Stirling-Formel auf der rechten Seite approximiert die Fakultät in dem folgenden Sinne: Es gilt
\\[0.3cm]
\hspace*{1.3cm}
$\ds \lim\limits_{n\rightarrow \infty} \frac{\sqrt{2 \cdot \pi \cdot n\;} \cdot \left(\frac{n}{\mathrm{e}}\right)^{n}}{n!} = 1$.
\\[0.3cm]
...
\end{array}
\end{equation}
Zu Berechnung der verschiedenen Fakultäten in dieser Formel wenden wir auf die Stirling-Formel den
Logarithmus an.  Das liefert
\\[0.1cm]
\hspace*{1.3cm}
$\ds \ln\bigl(n!\bigr) \approx  \frac{1}{2}\cdot\ln(2\cdot \pi \cdot n) + n \cdot \ln(n) - n$.
\\[0.1cm]
Setzen wir $\ln(k!)$, $\ln((n-k)!)$ und $\ln(n!)$ die entsprechenden Wert in Gleichung (\ref{eq:binappr}) ein, so  können wir den
Binomial-Koeffizienten approximieren.
Berechnen wir ${100 \choose 50}$ auf diese Weise, so erhalten wir 
\\[0.1cm]
\hspace*{1.3cm}
$\ds {100 \choose 50} \approx 101\,143\,884\,241\,463\,730\,000\,000\,000\,000$.
\\[0.1cm]
Der exakte Wert ist
\\[0.1cm]
\hspace*{1.3cm}
...
\\[0.1cm]
und der relative Fehler liegt bei $2.5$\textperthousand.  Bei der Berechnung von
...
\\[0.1cm]
Auch diesmal liegt der relative Fehler bei $2.5$\textperthousand.
...
\\[0.1cm]
auf.  
Für diesen Fall gibt es eine Approximations-Formel, die noch einfacher zu handhaben
...
\\[0.1cm]
Allgemein lässt sich eine Summe der Form $\sum_{i=0}^k f(i)$ wie folgt durch ein Integral
approximieren: 
\\[0.1cm]
\hspace*{1.3cm}
...
\begin{array}[t]{lcl}
 F^n_p(k) & =       & \ds 
                      \sum\limits_{i=0}^k {n \choose i} \cdot p^i \cdot (1 - p)^{n-i} \\[0.5cm]
          & \approx & \ds 
