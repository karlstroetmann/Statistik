...
\blue{Wahrscheinlichkeit} präzise festzulegen.  Dazu definieren wir zunächst den Begriff des \blue{Zufalls-Experiments}.
Unter einem \blue{Zufalls-Experiment} verstehen wir ein Experiment, dessen Ausgang nicht
...
Elementar-Ereignisse die selbe Wahrscheinlichkeit haben.  Diese Annahme ist  aus
...
$A \cup B$ dann aus den Wahrscheinlichkeiten der Ereignisse $A$ und $B$ berechnen läßt, wenn die
...
Bezeichen wir für eine Menge $M$ die Anzahl ihrer Elemente mit $|M|$, so gilt 
\\[0.2cm]
\hspace*{1.3cm}
$|A \cup B| = |A| + |B| - | A \cap B|$
\\[0.2cm]
Diese Formel läßt sich wie folgt interpretieren:
...
Die Gleichung (\ref{eq:u3}) läßt sich verallgemeinern.  Betrachten wir die Vereinigung
...
$
\\[0.2cm]
In ähnlicher Weise läßt sich eine Formel herleiten, die die Wahrscheinlichkeit einer
...
zu berechnen.  Manchmal ist es in solchen Fällen möglich, statt dessen die Wahrscheinlichkeit für das Ereignis
...
\item Vergabe einer 1: 
      ...
      \\[0.2cm]
      Die Menge $A_1$ besteht aus $6 + 6 - 1$ Elementen, denn 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\item Vergabe einer 2: 
      ...
      \\[0.2cm]
      Die Menge $A_2$ besteht aus $5 + 5 - 1$ Elementen.
...
\item Vergabe einer 3: 
      ...
      \\[0.2cm]
      Die Menge $A_3$ besteht aus $4 + 4 - 1$ Elementen.
...
\item Vergabe einer 4: 
      ...
      \\[0.2cm]
      Die Menge $A_4$ besteht aus $3 + 3 - 1$ Elementen.
...
\item Vergabe einer 5: 
      ...
      \\[0.2cm]
      Die Menge $A_5$ besteht aus $2 + 2 - 1$ Elementen.
...
\item Vergabe einer 6: 
      ...
      \\[0.2cm]
      Die Menge $A_6$ enthält nur ein Element.
      Also hat die Wahrscheinlichkeit für die Note 6 den Wert $\frac{1}{36} =
      0.02\overline{7}$.
      \hspace*{\fill} $\Box$
\end{enumerate}
...
die Kardinalitäten für diejenigen Mengen, die in der Praxis häufig vorkommen, berechnen.  
...
$k > n$ ist.
Der Ausdruck ${n \choose k}$ genügt der folgenden Rekursions-Gleichung:
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{Satz}[Vandermonde-Gleichung \label{satz:vandermonde}]
...
\proof
Zwar kann auch diese Gleichung algebraisch bewiesen werden, aber es ist instruktiver, wenn wir versuchen zu
...
\item $\ds {n \choose k} = {n \choose n - k}$
      ...
      \textbf{Bemerkung}: Diese Gleichung wird als Symmetrie-Eigenschaft der Binomial-Koeffizienten bezeichnet.
...
$A_k := \{ L \in \Omega \,|\, L[k] \not= k \}$.
\\[0.2cm]
Das Ereignis $E$, für das wir uns interessieren, ist das Ereignis, bei der alle Damen Glück haben, es gilt also
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
Als nächstes müssen wir uns überlegen, wie viele Möglichkeiten es gibt, um ein Menge
$I \subseteq \{1,\cdots,n\}$ mit $j$ zu bilden:  Dies ist die Anzahl der Teilmengen der Menge $\{1,\cdots,n\}$,
...
Multimengen werden ähnlich wir Mengen mit Hilfe von geschweiften Klammern notiert.  Zur Unterscheidung
...
\item Wie viele Möglichkeiten gibt es, ein Tripel natürliche Zahlen $\langle x, y,z \rangle$ zu wählen,
      ...
      $x + y + z = 5$
      \\[0.2cm]
      gilt?  Schreiben Sie ein \textsc{SetlX}-Programm, dass die Menge dieser Tripel explizit berechnet.
\item Es sei $s\in \mathbb{N}$.  Wie viele Möglichkeiten gibt es, $n$ natürliche Zahlen $x_1,x_2,\cdots,x_n$ so
      zu wählen, dass 
      \\[0.2cm]
      \hspace*{1.3cm}
      $\ds \sum\limits_{k=1}^n x_i = s$
      \\[0.2cm] 
      gilt?
...
\noindent
\textbf{Hinweis}:  Sie können ein \textsc{SetlX}-Programm schreiben, dass die Menge aller Tripel
...
\begin{Satz}[Würfel-Summen-Satz] \label{satz:wuerfel_anzahl}
...
\\[0.2cm]
\hspace*{1.3cm}
$\langle x_1, \cdots, x_n \rangle \in W(s) \;\Leftrightarrow\; \langle x_1-1, \cdots, x_n-n \rangle \in V(s-n)$,
\\[0.2cm]
...
\\[0.2cm]
Da die Mengen $A_i(t)$ Teilemengen der Menge $B(t)$ sind, folgt daraus für die Kardinalität von $V(t)$ die Formel  
\\[0.2cm]
\hspace*{1.3cm}
...
$
\\[0.2cm]
Der Name $\mathrm{ite}$ steht hier als Abkürzung für \textsl{if-then-else}.
\\[0.2cm]
\hspace*{1.3cm}
$
A_I'(t) := 
\bigl\{ \langle \mathrm{ite}(i\in I,x_1-6, x_1), \cdots, \mathrm{ite}(n\in I,x_n-6,x_n\rangle 
 \bigm| 
...
von der Mächtigkeit $k$ dieser Menge abhängt.  Da es insgesamt ${n \choose k}$ Möglichkeiten gibt, eine Menge
...
Geburtstag haben, läßt sich nicht unmittelbar angeben.  Wir betrachten daher zunächst das
...
\begin{figure}[!ht]
  \centering
   \epsfig{file=Abbildungen/birthday,scale=0.5}
...
farblichen Markierung ist.  Tabellieren Sie die Werte für $k=0,\cdots,20$ und tragen Sie
die gefundenen Werte in einem Diagramm auf.
...
$M$ eine Teilmenge der Größe $200$ auswählen.  Damit läßt sich die Ergebnis-Menge als
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
$\ds P(\Lambda_k) = \frac{|\Lambda_k|}{|\Omega|} =\frac{{1000 \choose k} \cdot {39\,000 \choose 200 - k}}{{40\,000 \choose 200}}$
\\[0.2cm]
...
\hline
  \end{tabular}}
  \caption{Wie viele bunte Ameisen werden gefangen?}
  \label{tab:ameisen}
\end{table}
...
\begin{figure}[!ht]
  \centering
   \epsfig{file=Abbildungen/ameisen,scale=0.5}
   \caption{Wahrscheinlichkeits-Maß für die Anzahl der markierten Ameisen}
  \label{fig:ameisen}
\end{figure}
...
insgesamt $k$ Bauteile defekt sind.  Die Situation ist dann die selbe wie in der letzten
Aufgabe:  
\begin{enumerate}
...
\\[0.2cm]
Das der Aufgabe zugrunde liegende Zufalls-Experiment besteht darin, dass wir aus der Menge
$\mathcal{M}$ eine Teilmenge der Größe $n$ auswählen.  Damit läßt sich die Ergebnis-Menge als
\\[0.2cm]
...
\begin{equation}
 ...
 P(\Lambda_k) = \frac{|\Lambda_k|}{|\Omega|} =\frac{{K \choose k} \cdot {N - K \choose n - k}}{{N \choose n}}
\end{equation}
...
\item Der zweite Tag ist schwieriger, denn hier müssten wir tatsächlich erst  $200$
      ...
      untersuchen und jedesmal überprüfen, ob die Ameise markiert ist.  
\end{enumerate}
Am zweiten Tag kann es jetzt durchaus passieren, dass wir die selbe Ameise mehrmal zählen.
...
\begin{array}[b]{lcll}
 ...
 & = & \ds \frac{{n \choose k} \cdot K^k \cdot (N-K)^{n-k}}{N^n} \\[0.4cm]
...
Beispielen schon mehrfach begegnet ist.  Dies ist der Begriff der Zufalls-Variable.
...
\begin{Definition}[Binomial-Verteilung]
  Wird ein Bernoulli-Experiment mit Parameter $p$ insgesamt $n$ mal so wiederholt, dass die einzelnen Ausführungen des Experiments
...
\begin{array}[t]{lcl}
 ...
 E[S] & = & \ds \sum\limits_{s=2}^{12} P(S=s)\cdot s \\[0.5cm]
      & = & \ds \frac{1}{36} \sum\limits_{s=2}^7 (s-1)\cdot s +
                          \frac{1}{36} \sum\limits_{s=8}^{12} (13-s)\cdot s \\[0.5cm]
      & = & \ds \frac{252}{36} \;=\; 7 
\end{array}
$
\\[0.2cm]
...
Würfeln sollte doppelt so groß sein. \qed
...
Wert $\frac{5}{6} \cdot \frac{1}{6}$.  Allgemein hat die Wahrscheinlichkeit dafür, dass im
$n+1$-ten Wurf eine 6 fällt, den Wert
\\[0.2cm]
\hspace*{1.3cm} $\ds \left(\frac{5}{6}\right)^n \cdot \frac{1}{6}$.
\\[0.2cm]
Bezeichen wir die Zufalls-Variable, die die Anzahl der benötigten Würfe angibt, mit $N$, so
...
\\[0.2cm]
Diese Summe können wir mit einem Trick auf die geometrische Reihe zurück führen.
...
\\[0.2cm]
Das Ergebnis ist intuitiv einleuchtend. \qed
...
\begin{Definition}[Varianz, Standard-Abweichung] \hspace*{\fill} \\
  ...
  \\[0.2cm]
  Im Gegensatz zur Varianz hat die Standard-Abweichung die selbe Einheit wie die Zufalls-Variable $X$.
  \qed
\end{Definition}

\example
Wir berechnen die Varianz der Zufalls-Variable $S$, die die {Augenzahl} beim
...
\\[0.2cm]
Führen wir die selbe Rechnung für das Experiment ``Würfeln mit  zwei Würfeln'' durch, so
...
\begin{array}[t]{lcl}
 ...
 \textsl{Var}[S] & = & \ds \sum\limits_{s=2}^{12} P(S=s)\cdot (s - 7)^2 \\[0.5cm]
      & = & \ds \frac{1}{36} \sum\limits_{s=2}^7 (s-1)\cdot (s - 7)^2 +
                          \frac{1}{36} \sum\limits_{s=8}^{12} (13-s)\cdot (s-7)^2 \\[0.5cm]
      & = & \ds \frac{35}{6}
\end{array}
$
\\[0.2cm]
