...
aller Frauen das Alter von mindestens 60 Jahren erreichen, während $57\,062$ ein
...
Ergebnisse nach wie vor die selbe Wahrscheinlichkeit haben, dann haben wir einen
...
$
\\[0.2cm]
Dieses Ergebnis läßt sich verallgemeinern.  Ist eine Familie $B_1$, $\cdots$, $B_n$
...
Personen hat dann $83$ Personen Malaria.  Würden wir nun alle Personen aus 
...
Täter handelt?  Beantworten Sie die selbe Frage für den Fall, dass auf der Insel $1\,000\,000$ Männer leben.
...
Show-Master die Wahl zufällig, wobei er beide Türen mit der selben Wahrscheinlichkeit wählt.  Der Kandidat
...
Werte von $a$ und $w$ fest sind.  Wir nehmen an, dass diese Ereignisse alle die selbe
...
haben festgelegt, dass der Show-Master in diesem Fall beide Türen mit der selben Wahrscheinlichkeit auswählt.
...
zeigen wir eine alternative Lösung des Ziegen-Problems, bei der wir den zugrunde liegenden
...
\item $A_n$: ``\blue{Das Auto steht hinter der Tür mit der Nummer $n$}''.
      ...
      Da die Ereignisse $A_1$, $A_2$ und $A_3$ alle die selbe Wahrscheinlichkeit haben,
      gilt \\[0.2cm]
      \hspace*{1.3cm} $\ds P(A_n) = \frac{1}{3}$ \quad für alle $n \in \{1,2,3\}$.
\item $W_n$: ``\blue{Der Kandidat hat die Tür mit der Nummer $n$ gewählt}''.

      Da die Ereignisse $W_1$, $W_2$ und $W_3$ alle die selbe Wahrscheinlichkeit haben,
      gilt \\[0.2cm]
...
\item $O_n$: ``\blue{Der Showmaster hat die Tür mit der Nummer $n$ geöffnet}''.
      ...
      Die Wahrscheinlichkeit $P(O_n)$ läßt sich nicht unmittelbar angeben,
      denn das Ereignis $O_n$ wird offenbar von den anderen Ereignissen beeinflußt.
\end{enumerate}
...
$
\\[0.2cm] 
Das ist das selbe Ergebnis, was wir auch schon früher gefunden haben.
\eox

\exercise
Anton, Bruno und Charlie haben sich unsterblich in die selbe Frau verliebt und
...
Möglichkeit, in die Luft zu schießen.  Dies gilt allerdings nicht für Anton, der darf
nicht in die Luft schießen.  
...
\item Wie groß ist der Erwartungswert für die Dauer des Duells.
\end{enumerate}

\noindent
\textbf{Hinweis}: Die am schwierigsten zu berechnende Situation ist die,
wenn Bruno den Anton erschossen hat und Charlie und Bruno sich nun abwechselnd beschießen.
Um die bei diesem Duell auftretenden Wahrscheinlichkeiten berechnen zu können, ist
es zweckmäßig, für dieses Duell die folgenden Abkürzungen einzuführen.
\begin{enumerate}
\item $c(n)$ ist die Wahrscheinlichkeit dafür, dass Charlie nach der $n$-ten Runde noch lebt.

      Eine Runde besteht dabei aus dem Schuss von Charlie und dem Schuss von Bruno, falls
      dieser noch dazu in der Lage ist.
\item $b(n)$ ist die Wahrscheinlichkeit dafür, dass Bruno nach der $n$-ten Runde noch lebt.
\item $g(n)$ ist die Wahrscheinlichkeit dafür, dass das Spiel nach der $n$-ten Runde beendet ist.

      Die Wahrscheinlichkeit $g$ kann auf die beiden Wahrscheinlichkeiten $c(n)$ und $b(n)$
      zurückgeführt werden.
\end{enumerate}
Es ist zweckmäßig, für $c(n)$ und $b(n)$ ein System von Rekurrenz-Gleichungen aufzustellen und zu lösen.
Anschließend können die Grenzwerte dieser Folgen für $n$ gegen Unendlich berechnet werden. Es ergibt sich 
\\[0.2cm]
\hspace*{1.3cm}
...
das selbe Ergebnis bekommen.  Daher sind die beiden Zufalls-Variablen $X$ und $Y$ unabhängig. \eox
...
\item $X: \Omega \rightarrow \mathbb{R}$ eine Zufallsgröße auf $\mathcal{W}$.
\end{enumerate}
...
\\[0.2cm]
gilt. Bei der Auswertung der Zufallsgröße $X$ spielt also die zweite Komponente keine Rolle.
...
gilt. Den Nachweis, dass diese beiden Mengen  gleich sind führen wir, indem wir zeigen, dass 
\\[0.2cm]
\hspace*{1.3cm}
...
gegeben, wobei wir stillschweigend voraussetzen, dass die Elemente $c_n$ voneinander verschieden sind,
 und ist weiter $Z$ eine Zufallsgröße auf $\Omega$, so kann die Wahrscheinlichkeit
...
Der Erwartungswert hat die folgende Linearitäts-Eigenschaft.
...
und $Y: \Omega \rightarrow \mathbb{R}$ zwei Zufalls-Variablen, so können wir für beliebige $\alpha,\beta \in \mathbb{R}$ eine Zufallsgröße 
$Z:\Omega \rightarrow \mathbb{R}$ durch 
\\[0.2cm]
\hspace*{1.3cm}
$Z(\omega) := \alpha \cdot X(\omega) + \beta \cdot Y(\omega)$
\\[0.2cm]
definieren.  Für den Erwartungswert dieser Zufallsgröße gilt: 
\\[0.2cm]
\hspace*{1.3cm}
...
für ein beliebiges $\beta \in \mathbb{R}$ die Zufallsgröße $X:\Omega \rightarrow \mathbb{R}$ als 
\\[0.2cm]
...
ein Zufallsgröße, so können wir für beliebige $\alpha,\beta \in \mathbb{R}$ eine Zufallsgröße 
$Z:\Omega \rightarrow \mathbb{R}$ durch 
\\[0.2cm]
\hspace*{1.3cm}
$Z(\omega) = \alpha \cdot X(\omega) + \beta$
\\[0.2cm]
definieren.  Für die Varianz dieser Zufallsgröße gilt: 
\\[0.2cm]
\hspace*{1.3cm}
...
Erwartungswerts des Produktes zweier unabhängiger Zufalls-Variablen.
...
\\[0.2cm]
gegeben.  Dann ist der Wertebereich der Zufallsgröße $Z:\Omega \rightarrow \mathbb{R}$, die durch 
\\[0.2cm]
\hspace*{1.3cm}
...
ist die Zufallsgröße $Z:\Omega \rightarrow \mathbb{R}$ definiert durch 
\\[0.2cm]
\hspace*{1.3cm}
$Z(\omega) = X(\omega) + \alpha$,
\\[0.2cm]
so sind auch die Zufalls-Variablen $Z$ und $Y$ unabhängig. \eox
...
\begin{array}[b]{lcl}
 \var[X+Y] & = & E\bigl[(X + Y - \mu_X - \mu_Y)^2\bigr] \\[0.2cm] 
           ...
           &   & \mbox{nach Lemma \ref{lemma:produnabhaengig}, denn $X- \mu_X$ und $Y - \mu_Y$ sind unabhängig nach Aufgabe 19} \\[0.2cm]
...
\noindent
Der letzte Satz läßt sich auf Mengen von unabhängigen Zufalls-Variablen verallgemeinern.
Ist $\{X_1,\cdots,X_n\}$ eine Menge unabhängiger Zufalls-Variablen, so gilt 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
beschrieben wird.  Weiter sei $X$ eine Zufallsgröße, die bei diesem Experiment eine Rolle spielt.
Wiederholen wir das Zufalls-Experiment $n$-mal, so haben wir eine Menge 
$\{X_1, \cdots, X_n\}$ von $n$ Zufalls-Variablen, die offenbar unabhägig sind.  Wir definieren jetzt die Zufallsgröße
\\[0.2cm]
\hspace*{1.3cm}
...
Wesentlich interessanter ist die Frage nach der Varianz der Zufallsgröße $\overline{X}$. 
Es gilt 
\\[0.2cm]
\hspace*{1.3cm}
$
...
ist dann der tatsächliche Wert, während die Standard-Abweichung als die ungefähre Größe des Meßfehlers 
interpretiert werden kann.  Das $\sqrt{n}$-Gesetz zeigt uns eine Möglichkeit, um die Genauigkeit
unserer Messung zu verbessern:  Führen wir die selbe Messung $n$-mal durch und bilden den Mittelwert unserer 
Ergebnisse, so wird die Meß-Genauigkeit um den Faktor $\sqrt{n}$ gesteigert. 
Ein anderes Beispiel ist das Würfeln:  Gibt die Zufallsgröße $X$ die Augenzahl beim einmaligen
...
\section[Das Gesetz der großen Zahlen]{Der Satz von Tschebyschow und das Gesetz  der großen Zahlen}
...
\begin{Satz}[Ungleichung von Tschebyschow]
  ...
  \begin{enumerate}
  \item $\langle \Omega, 2^\Omega, P \rangle$ ein Wahrscheinlichkeits-Raum,
  \item $X: \Omega \rightarrow \mathbb{R}$ eine Zufallsgröße mit Erwartungswert $\mu = E[X]$ und
...
$
\\[0.2cm]
Im letzten Schritt haben wir ausgenutzt, dass die Summe nur über die $\omega$ läuft, für die $\left(\frac{X - \mu}{\sigma}\right)^2 \geq r^2$ gilt, so dass die Summe
größer wird, wenn wir $r^2$ durch $\left(\frac{X - \mu}{\sigma}\right)^2$ ersetzen.
...
\\[0.2cm]
Die Standard-Abweichung $\sigma$ können wir aus dem $\sqrt{n}$-Gesetz bestimmen, denn da
...
\exercise
Angenommen, wir würfeln $n$ mal mit einem Laplace-Würfel.  Bestimmen Sie mit Hilfe der Ungleichung von
Tschebyschow ein $n$ so,  dass Sie mit einer  Wahrscheinlichkeit von $99\%$ davon ausgehen können, 
dass mindestens $\frac{n}{7}$ mal eine Sechs  gewürfelt wird? \eoxs

\remark
Sie können diese Aufgabe auch mit Hilfe der Näherung von Laplace für die (kumulative) Verteilungsfunktion 
...
\noindent
Wenn wir ein Zufalls-Experiment nur oft genug durchführen und dabei eine Zufallsgröße 
...
\begin{enumerate}
\item $\mathcal{W} = \langle \Omega, 2^\Omega, P \rangle$  ein Wahrscheinlichkeits-Raum,
\item $X: \Omega \rightarrow \mathbb{R}$ eine Zufallsgröße auf $\mathcal{W}$ mit dem Erwartungswert $\mu = E[X]$ und Standard-Abweichung
      $\sigma := \sqrt{\var[X]\,}$,
\item $\mathcal{W}^n = \underbrace{\mathcal{W} \times \cdots \times \mathcal{W}}_n$
      sei das $n$-fache kartesisches Produkt von $\mathcal{W}$ mit sich selbst,
\item $X_i: \Omega^n \rightarrow \mathbb{R}$ sei für $i=1,\cdots,n$ definiert durch 
...
\noindent
\textbf{Beweis}: Nach dem $\sqrt{n}$-Gesetz haben wir für die Zufallsgröße $\overline{X}$:
\\[0.2cm]
\hspace*{1.3cm}
