...
\remark
Forensiche DNA-Tests sind keinesweges so sicher, wie dies in der ländlichen Bevölkerung gemeinhin angenommen
...
Zusammenfassend läßt sich sagen, dass es sich bei den DNA-Tests ähnlich verhält, wie bei bei den Gottesurteilen
...
gefunden wurde. Obwohl Lukis Anderson auf Grund übermäßigen Alkohlkonsums zum Zeitpunkt des Mordes bewußtlos im
Krankenhaus lag, musste er trotzdem fünf Monate im Gefängnis verbringen, bis herauskam, dass die selben
...
Anderson angeschlossen.  Das selbe Gerät wurde später auch an den Fingern des Mordopfers angeschlossen.  Mehr
dazu können Sie  
...
\item Angenommen, Sie wissen, dass eines der Kinder im Winter geboren wurde und außerdem ein Junge ist. 
      ...
      Jahreszeiten die Selbe ist.  \eox
\end{enumerate}
...
Anschließend können die Grenzwerte dieser Folgen für $n$ gegen Unendlich berechnet werden.
% Es ergibt sich 
% \\[0.2cm]
% \hspace*{1.3cm}
...
sind, es gilt also $m \not= n \rightarrow c_m \not= n$,  und ist weiter $Z$ eine Zufallsgröße auf $\Omega$, so kann die Wahrscheinlichkeit
...
ist die Zufallsgröße $Z:\Omega \rightarrow \mathbb{R}$ durch 
\\[0.2cm]
\hspace*{1.3cm}
...
Wir definieren jetzt die Zufallsgröße 
\\[0.2cm]
\hspace*{1.3cm}
...
\section[Das Gesetz der großen Zahlen]{Satz von Tschebyscheff und Gesetz  der großen Zahlen}
Der folgende \href{https://de.wikipedia.org/wiki/Tschebyscheff-Ungleichung}{Satz}, der nach 
\href{https://de.wikipedia.org/wiki/Pafnuti_Lwowitsch_Tschebyscheff}{Pafnuti Lwowitsch Tschebyscheff} 
...
\begin{Satz}[Ungleichung von Tschebyscheff]
  ...
  \begin{enumerate}
  \item $\langle \Omega, 2^\Omega, P \rangle$ ein Wahrscheinlichkeits-Raum,
  \item $X: \Omega \rightarrow \mathbb{R}$ eine Zufallsgröße mit dem Erwartungswert $\mu = E[X]$ und der
...
\sum\limits_{\omega \in \Omega} P\bigl(\{\omega\}\bigr) \cdot \left(\frac{X - \mu}{\sigma}\right)^2 
      \\[0.7cm]
& = & \ds
\frac{1}{\sigma^2} \cdot\sum\limits_{\omega \in \Omega} P\bigl(\{\omega\}\bigr) \cdot (X - \mu)^2 
      \\[0.7cm]
& = & \ds
...
\\[0.2cm]
gilt.  Um diesen Ausdruck auf die Ungleichung von Tschebyscheff zurück führen zu können, formen wir den Wert $0.1$ um:
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
schreiben.  Damit diese Ungleichung aus der Ungleichung von Tschebyscheff folgt, muss 
\\[0.2cm]
\hspace*{1.3cm}
...
\remark
Die Ungleichung von Tschebyscheff ist nicht sehr genau.  Wir werden später noch eine wesentlich bessere
Abschätzung finden.  \eoxs

\exercise
Angenommen, wir würfeln $n$ mal mit einem Laplace-Würfel.  Bestimmen Sie mit Hilfe der Ungleichung von
Tschebyscheff ein $n$ so,  dass Sie mit einer  Wahrscheinlichkeit von $99\%$ davon ausgehen können, 
...
lösen.  Dann ergibt sich ein kleinerer Wert.  Das ist nicht weiter verwunderlich, denn die Tschebyscheff-Ungleichung
...
\begin{enumerate}
\item $\mathcal{W} = \langle \Omega, 2^\Omega, P \rangle$  ein Wahrscheinlichkeits-Raum,
\item $X: \Omega \rightarrow \mathbb{R}$ eine Zufallsgröße auf $\mathcal{W}$ mit dem Erwartungswert $\mu =
...
\\[0.2cm]
und mit nach der Ungleichung von Tschebyscheff folgt
\\[0.2cm]
\hspace*{1.3cm}
